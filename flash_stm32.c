			    ...
			    size_t len)
{
	...
	//LOG_INF("FLASH_STM32_BASE_ADDRESS is %x",FLASH_STM32_BASE_ADDRESS);
			     ...
			     size_t len)
{
	...
	LOG_DBG("rc value is %d",rc);
	if (rc == 0) {
		//LOG_INF("In if condition");
	...
	//LOG_INF("succesfully of if condition 215 rc is %d",rc);
	//LOG_INF("rc and rc2 value is %d and %d",rc,rc2);
	if (!rc) {
		rc = rc2;
	}
...
//LOG_DBG("FLASH_SECURITY_NS is set");
#else
//LOG_DBG("FLASH SECURITY is not set");
#endif
#if defined(FLASH_CR_LOCK)
//LOG_DBG("FLASH_CR_LOCK is set");
#else
//LOG_DBG("FLASH CR LOCK is not set");
#endif
//LOG_DBG("The value of dev is %p",dev);
	FLASH_TypeDef *regs = FLASH_STM32_REGS(dev);
	int rc = 0;
...
//LOG_DBG("FLASH CR LOCK is not set");
	...
	//LOG_DBG("Under FLASH_CR_LOCK");
	//LOG_DBG("Value of FLASH_CR_LOCK is %p",FLASH_CR_LOCK);
	if (enable) {
		regs->CR |= FLASH_CR_LOCK;
	} else {
	//	LOG_DBG("In the else condition");
	//	LOG_DBG("the value of regs->CR is %x",regs->CR);
	//	LOG_DBG("The value of regs->CR & FLASH_CR_LOCK is %x",regs->CR & FLASH_CR_LOCK);
		if (regs->CR & FLASH_CR_LOCK) {
	//		LOG_DBG("Line 318 in the if con");
			regs->KEYR = FLASH_KEY1;
			regs->KEYR = FLASH_KEY2;
	//		LOG_DBG("Unlocked regs");
		}
	}
#else
	if (enable) {
	...
	if (enable) {
		...
		//LOG_DBG("Srikar: I am enabling the write protection now");
		//regs->CR |=1<<31;
		//LOG_INF("value of regs->CR is %x",regs->CR);
		//LOG_INF("Enable write protection");
	} else {
		LOG_DBG("Disable write protection");
		//LOG_DBG("Srikar: I am disabling the write protection now");
		//regs->CR &=(0x7FFFFFFFF);
		//LOG_INF("value of regs->CR is %x",regs->CR);
	}
