...
{
	LOG_DBG("ACR value is %x",regs->ACR);
	if (regs->ACR & FLASH_ACR_DCEN) {
		LOG_DBG("Inside Line 62");
...
static int erase_sector(const struct device *dev, uint32_t sector)
{
	...
	uint32_t tmp;
	int rc;
	//LOG_DBG("printing regs values");
	//LOG_DBG("ACR value is %x",regs->ACR);
	//LOG_DBG("KEYR value is %x",regs->KEYR);
	//LOG_DBG("OPTKEYR value is %x",regs->OPTKEYR);
	//LOG_DBG("SR value is %x",regs->SR);
	//LOG_DBG("CR value is %x",regs->CR);
	//LOG_DBG("OPTC value is %x",regs->OPTCR);
	/* if the control register is locked, do not fail silently */
	if (regs->CR & FLASH_CR_LOCK) {
		return -EIO;
	}
	//LOG_DBG("No fault from line 153 so no -EIO");
	rc = flash_stm32_wait_flash_idle(dev);
	//LOG_DBG("rc value line 155 is %d",rc);
	if (rc < 0) {
		return rc;
	}
...
#endif
	//LOG_DBG(" now in line 181");
	//LOG_DBG("CR value is %x",regs->CR);
	//LOG_DBG("the value CR_PSIZE_MASK is %x",CR_PSIZE_MASK);
	//LOG_DBG("the value FLASH_PROGRAM_SIZE id is %x",FLASH_PROGRAM_SIZE);
	regs->CR &= CR_PSIZE_MASK;
	//regs->CR |= FLASH_PROGRAM_SIZE;
	regs->CR |= 0x200;
	//LOG_DBG("CR value is %x",regs->CR);
	//LOG_DBG("the sector value is %d",sector);
	//LOG_DBG("FLASH_CR_SNB value is %x",FLASH_CR_SNB);
	//LOG_DBG("FLASH_CR_SER value is %x",FLASH_CR_SER);
	//LOG_DBG("FLASH_CR_STRT value is %x",FLASH_CR_STRT);
	regs->CR &= ~FLASH_CR_SNB;
	//LOG_INF("CR value is %x",regs->CR);
	...
	/* flush the register write */
	tmp = regs->CR;
	// LOG_INF("tmp value is %x",tmp);
	// LOG_INF("printing regs values");
	// LOG_INF("ACR value is %x",regs->ACR);
	// LOG_INF("KEYR value is %x",regs->KEYR);
	// LOG_INF("OPTKEYR value is %x",regs->OPTKEYR);
	// LOG_INF("SR value is %x",regs->SR);
	// LOG_INF("CR value is %x",regs->CR);
	// LOG_INF("OPTC value is %x",regs->OPTCR);
	
	rc = flash_stm32_wait_flash_idle(dev);
	regs->CR &= ~(FLASH_CR_SER | FLASH_CR_SNB);
	// LOG_INF(" at line 195 CR value is %x",regs->CR);
	return rc;
}
				 ...
				 unsigned int len)
{
	...
	uint32_t i;
	int rc = 0;
	//LOG_DBG("in the flash_stm32f4x.c file");
	rc = flash_get_page_info_by_offs(dev, offset, &info);
	//LOG_DBG("in flash_stm32f4.c file rc value is %d",rc);
	if (rc) {
		return rc;
	}
	start_sector = info.index;
	//LOG_DBG("the value of start_sector is %d",start_sector);
	rc = flash_get_page_info_by_offs(dev, offset + len - 1, &info);
	//LOG_DBG("in flash_stm32f4.c file  line 204 rc value is %d",rc);
	if (rc) {
		return rc;
	}
	end_sector = info.index;
	//LOG_DBG("the value of end_sector is %d",start_sector);
